    ...
    <>
      ...
      <div
        className={`flex flex-col h-full ${
          isCollapsed ? "w-[80px] px-1 items-center" : "w-[300px] px-4"
        } py-4 border-r border-gray-300 bg-[#F8F9FC] overflow-y-scroll transition-all duration-300 relative ${
          isMobile && !isCollapsed ? "fixed left-0 top-0 z-50 shadow-2xl" : ""
        } ${isMobile && isCollapsed ? "fixed left-0 top-0 z-10" : ""}`}
      >
        <button
          onClick={() => setIsCollapsed(!isCollapsed)}
          className="bg-[#E9EFFF] text-blue-600 rounded p-1 hover:bg-blue-100 transition-colors absolute top-4 right-0 rounded-tl-full rounded-bl-full z-10"
        >
          {!isCollapsed ? (
            <ChevronLeft size={12} />
          ) : (
        ...
        >
          ...
          {!isCollapsed && (
            <input
              ...
              className="outline-none ml-[10px] placeholder:text-gray-400 w-full"
            />
          )}
        </div>
          ...
          <div>
            ...
            {!isCollapsed && (
              <span className="text-xs">
                Upgarde For smarter Ai and more...
              </span>
            )}
          </div>
          {!isCollapsed && (
        ...
        >
          ...
          {!isCollapsed && (
            <img src="usermenu.png" className="h-[25px] w-[25px]" alt="Menu" />
          )}
        </div>
      </div>
    </>
